...
for cur_instr_dir in "$test_directory"/* ; do
    ...
    for test_file in "$cur_instr_dir"/*.c; do
        ...
        for flag in "${optimization_flags[@]}"; do
           ...
           cur_asm=${asm_dir%.*}-$flag.s # change extension and add optimization flag
           
            if [ "$platform" == "Linux" ]; then
                $compiler -march="rv64gc_zba_zbb_zbc_zbs" -S -o "$cur_asm" "$flag" "$test_file"
            ...
            if [ $? -eq 0 ]; then
                ...
                if [ "$platform" == "Linux" ]; then
                    objdump -d "$cur_asm" | grep -oP "(?<=<)[^>]+" | grep "$instr_name" | while read -r line; do
        		if [ "$line" == "$instr_name" ] && ! nm -D "$asm_dir" | grep -q "\<$instr_name\>"; then
            			echo "$line" >> $output_file
        		fi
    		done
	    	elif [ "$platform" == "Darwin" ]; then
    			otool -tv "$cur_asm" | grep -oP "(?<=<)[^>]+" | grep "$instr_name" | while read -r line; do
